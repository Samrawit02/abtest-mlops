...
'''
This is a simple script for computing evaluation metrics for random forest model
'''
...
model = pickle.load(open(str(Config.MODELS_PATH / "random_forest_model.pickle"), "rb"))

y_pred = model.predict(X_test)
evaluate = EvaluateModel(y_test, y_pred)
metrics = evaluate.get_metrics()

with open(str(Config.METRICS_FILE_PATH / "randomforest_metrics.json"), "w") as outfile:
